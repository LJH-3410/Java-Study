...
public class firstmodel extends JFrame implements Runnable, KeyListener {
   ...
   public void ultimatecrashChk() {    //필살기 맞았을때 적이 없어지고 점수 오르게 하는 것
                Graphics g = this.getGraphics();
                Polygon p = null;
                for(int i = 0; i < ultimateList.size(); i++) {
                   Us u = (Us)ultimateList.get(i);
                     for(int j = 0; j < enList.size(); j++) {
                         Enemy e = (Enemy)enList.get(j);
                         int[] xpoints = {u.x, (u.x + u.w+1000), (u.x + u.w-1000), u.x}; // 필살기 범위가 전 범위로 설정
   ...
   public void draw() { // 창 안에 폰트 설정, 기본 색 설정, 점수와 목숨 표기 
      ...
      for(int i = 0; i < msList.size(); i++) { // 기본 총알의 위치 설정
         Ms m = (Ms)msList.get(i);
      ...
      for(int i = 0; i < ultimateList.size(); i++) { // 필살기 위치 설정
              Us u = (Us)ultimateList.get(i);
              gs.setColor(Color.blue);
   ...
   public void keyReleased(KeyEvent ke) { // 기본 키 타입
      switch(ke.getKeyCode()) {
      case KeyEvent.VK_LEFT:
         left = false;
         break;
